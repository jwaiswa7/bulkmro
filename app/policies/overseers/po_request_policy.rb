...
class Overseers::PoRequestPolicy < Overseers::ApplicationPolicy
  ...
  def sending_po_to_supplier_new_email_message?
    (record.status == 'PO Created' || record.status == 'Supplier PO Sent') && record.purchase_order && record.contact.present?
  end
  ...
  def dispatch_supplier_delayed_new_email_message?
    (record.status == 'PO Created' || record.status == 'Supplier PO Sent') && (admin? || logistics?) && record.purchase_order && record.contact.present?
  end
  ...
  def material_received_in_bm_warehouse_new_email_msg?
    (record.status == 'PO Created' || record.status == 'Supplier PO Sent') && (admin? || logistics?) && record.purchase_order && record.contact.present? && record.purchase_order.material_status.present?
  end
