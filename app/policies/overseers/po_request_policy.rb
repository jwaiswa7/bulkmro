class Overseers::PoRequestPolicy < Overseers::ApplicationPolicy
  ...
  def new_email_message?
    (admin? || sales?) && record.status == "PO Created" && record.purchase_order.has_supplier? && record.purchase_order.get_supplier(record.purchase_order.rows.first.metadata['PopProductId'].to_i).company_contacts.present?
  end
