json.data (@inquiries) do |inquiry|
  columns = [
      ...
      inquiry.account.present? ? conditional_link(inquiry.account.to_s, overseers_account_path(inquiry.account), policy(inquiry.account).show?) : '-',
      inquiry.company.present? ? conditional_link(inquiry.company.to_s, overseers_company_path(inquiry.company), policy(inquiry.company).show?) : '-',
      inquiry.contact.present? ? conditional_link(inquiry.contact.to_s, overseers_contact_path(inquiry.contact), policy(inquiry.contact).show?) : '-',
