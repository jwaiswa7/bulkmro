  ...
  json.array! [
                  ...
                  [
                      ...
                      if is_authorized(:delivery_challan, 'create') && (policy(inward_dispatch).create_new_dc?)
                        row_action_button_without_fa(new_overseers_delivery_challan_path(inward_dispatch: inward_dispatch, inquiry_id: inward_dispatch.purchase_order.inquiry_id, created_from: inward_dispatch.class), 'bmro-plus-circle-icon', 'New Delivery Challan', 'success', target: :_blank)
                      end
                  ].join(' '),
