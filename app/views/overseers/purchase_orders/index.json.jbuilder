json.data (@purchase_orders) do |purchase_order|
  json.array! [
                  ...
                  (purchase_order.supplier.present? ? conditional_link(purchase_order.supplier.try(:name), overseers_company_path(purchase_order.supplier), policy(purchase_order.inquiry).show?) : '-' if purchase_order.rows.present?),
                  format_star((purchase_order.supplier.try(:rating) if purchase_order.rows.present?)),
