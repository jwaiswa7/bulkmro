json.data (@kits) do |kit|
  json.array! [
                  [
                      if is_authorised(kit, 'show')
                        row_action_button(overseers_kit_path(kit), 'eye', 'View Kit', 'info', :_blank)
                      end,
                      if is_authorised(kit, 'edit')
                        row_action_button(edit_overseers_kit_path(kit), 'pencil', 'Edit Kit', 'warning', :_blank)
                      end
                  ].join(' '),
                  conditional_link(kit.product.present? ? kit.product.name : '-', overseers_kit_path(kit), is_authorised(kit.product, 'edit')),
                  kit.product.sku,
                  conditional_link(kit.inquiry.inquiry_number, edit_overseers_inquiry_path(kit.inquiry), is_authorised(kit.inquiry, 'edit')),
