  ...
  json.array! [
                  [
                      ...
                      if policy(customer_order).edit?
                        row_action_button(new_overseers_inquiry_path(company_id: customer_order.company.to_param, customer_order_id: customer_order.to_param), 'plus-circle', 'Create Inquiry', 'success')
                      end
                  ].join(' '),
