  ...
  json.array! [
                  [
                      if po_request.sales_order.present? && (is_authorized(po_request, 'edit'))
                        row_action_button_without_fa(edit_overseers_po_request_path(po_request), 'bmro-icon-table bmro-icon-pencil', 'Edit PO Request', 'warning')
                      elsif is_authorized(po_request, 'edit') && po_request.status != 'Cancelled'
                        row_action_button_without_fa(edit_overseers_inquiry_po_request_path(po_request.inquiry, po_request), 'bmro-icon-table bmro-icon-pencil', 'Edit PO Request', 'warning')
                      end,
                      if is_authorized(po_request, 'can_cancel') && policy(po_request).can_cancel?
                        link_to('', class: ['btn btn-sm btn-dark cancel-po_request'], 'data-po-request-id': po_request.id, title: 'bmro-icon-table bmro-icon-pencil', remote: true) do
                      ...
                      if is_authorized(po_request, 'new_payment_request') && policy(po_request).new_payment_request?
                        row_action_button_without_fa(new_overseers_po_request_payment_request_path(po_request), 'bmro-icon-table bmro-icon-dollar', 'Payment Request', 'success', :_blank)
                      elsif is_authorized(po_request, 'show_payment_request') && po_request.payment_request.present?
                        row_action_button_without_fa(overseers_payment_request_path(po_request.payment_request), 'bmro-icon-table bmro-icon-used-view', 'View Payment Request', 'info', :_blank)
                      end,
                      if is_authorized(po_request, 'sending_po_to_supplier_new_email_message') && policy(po_request).sending_po_to_supplier_new_email_message? && current_overseer.smtp_password.present?
                        row_action_button_without_fa(sending_po_to_supplier_overseers_po_request_email_messages_path(po_request), 'bmro-icon-table bmro-icon-used-envelop', 'Send Purchase Order to Supplier', 'dark', :_blank)
                      else
                        row_action_button_without_fa(sending_po_to_supplier_overseers_po_request_email_messages_path(po_request), 'bmro-icon-table bmro-icon-used-envelop', 'Enter SMTP settings', 'dark disabled')
                      end,
                      if is_authorized(po_request, 'material_received_in_bm_warehouse_new_email_msg') && policy(po_request).material_received_in_bm_warehouse_new_email_msg? && current_overseer.smtp_password.present?
                        row_action_button_without_fa(material_received_in_bm_warehouse_overseers_po_request_email_messages_path(po_request), 'bmro-icon-table bmro-icon-used-envelop', 'Material Received in BM Warehouse', 'warning', :_blank)
                      else
                        row_action_button_without_fa(material_received_in_bm_warehouse_overseers_po_request_email_messages_path(po_request), 'bmro-icon-table bmro-icon-used-envelop', 'Enter SMTP settings', 'warning disabled')
                      end, '<br/>', '<br/>',
                      if is_authorized(po_request, 'new_purchase_order') && policy(po_request).new_purchase_order? && po_request.purchase_order_id.nil?
                        row_action_button_without_fa(new_purchase_order_overseers_po_request_path(po_request), 'bmro-icon-table bmro-icon-cricle', 'Create New Purchase Order', 'success')
                      end,
                      if po_request.status == 'Supplier PO: Amendment Pending' && is_authorized(po_request, 'po_amended')
                        row_action_button_without_fa(overseers_po_request_path(po_request), 'bmro-icon-table bmro-icon-used-view', 'Amendment Changes', 'success')
                      end
                  ].join(' '),
