                  ...
                  [
                      if po_request.sales_order.present? && (policy(po_request).edit?)
                        ...
                        [row_action_button(edit_overseers_inquiry_po_request_path(po_request.inquiry, po_request), 'pencil', 'Edit PO Request', 'warning', :_blank),
                           # row_action_button(overseers_po_requests_path(po_request), 'close-circle-outline', 'Cancel PO Request', 'warning', :_blank)
                        ]
                      end,
                      ...
                      if po_request.status.present? && !po_request.status.downcase.include?('cancel') && policy(po_request).edit?
