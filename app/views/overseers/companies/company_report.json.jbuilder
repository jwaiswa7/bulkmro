json.data (@indexed_company_reports) do |company|
  ...
  json.array! [
                  ...
                  link_to_if(company['live_inquiries']['value'].to_i != 0, number_with_delimiter(company['live_inquiries']['value'].to_i, delimiter: ','), filtered_path(overseers_inquiries_path, [filter_by_value('Company', Company.find(company['key']).name, Company.find(company['key']).id), filter_by_date_range('Date', @date_range), filter_by_value('Inside+Sales', procurement_specialist, procurement_specialist), filter_by_value('Outside+Sales', outside_sales_owner, outside_sales_owner)]), target: '_blank'),
                  ...
                  if @date_range.present?
                    link_to_if(company['expected_orders']['value'].to_i != 0, number_with_delimiter(company['expected_orders']['value'].to_i, delimiter: ','), filtered_path(overseers_inquiries_path, [filter_by_value('Company',  Company.find(company['key']).name, Company.find(company['key']).id), filter_by_value('Status', 'Expected Order', 7), filter_by_value('Inside+Sales', procurement_specialist, procurement_specialist), filter_by_value('Outside+Sales', outside_sales_owner, outside_sales_owner)]), target: '_blank')
                  else
                  ...
                  link_to_if(company['sales_orders']['value'].to_i != 0, number_with_delimiter(company['sales_orders']['value'].to_i, delimiter: ','), filtered_path(overseers_sales_orders_path, [filter_by_value('Account', Company.find(company['key']).account.to_s, Company.find(company['key']).account.id), filter_by_value('Inside+Sales', procurement_specialist, procurement_specialist), filter_by_value('Outside+Sales', outside_sales_owner, outside_sales_owner)]), target: '_blank'),
