json.data (@indexed_company_reports) do |company|
  ...
  account = Company.find(company['key']).account
  ...
  date_range = params['company_report'].present? ? params['company_report']['date_range'] : ''
  json.array! [
                  [],
                  link_to(record.account.to_s, overseers_account_path(record.account), target: '_blank', 'data-account-target': account.get_monthly_target(date_range)),
