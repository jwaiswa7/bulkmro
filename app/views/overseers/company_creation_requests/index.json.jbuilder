json.data (@company_creation_requests) do |company|
  json.array! [
                ...
                if !company.company.present? && is_authorized(:company, 'new')
                  row_action_button(new_overseers_company_path(ccr_id: company.id), 'bmro-icon-table bmro-icon-new-company', 'New Company', 'success', :_blank)
                end,
                if is_authorized(company, 'show')
                  row_action_button(overseers_company_creation_request_path(company), 'bmro-icon-table bmro-icon-used-view',  'View Company Creation Request', 'info', :_blank)
                end,
                if is_authorized(company.activity, 'show')
                  row_action_button(edit_overseers_activity_path(company.activity), 'bmro-icon-table bmro-icon-pencil',  'Edit Activity', 'warning', :_blank)
                end
            ].join(' '),
