json.data (@sales_receipts) do |sales_receipt|
  json.array! [
                  ...
                  sales_receipt.sales_invoices.map { |invoice| link_to(invoice.invoice_number, overseers_inquiry_sales_invoice_path(invoice.inquiry, invoice), target: '_blank') }.compact.join(' '),
                  sales_receipt.sales_invoices.map { |invoice| format_succinct_date(invoice.created_at) }.compact.join('<br />'),
                  ' - ',
