  ...
  json.array! [
                  [
                      if policy(pending_sales_order).comments?
                        row_action_button(overseers_inquiry_comments_path(pending_sales_order.inquiry, sales_order_id: pending_sales_order.to_param), 'comment-lines', pending_sales_order.inquiry.last_comment || 'See Comments', 'dark')
                      end,
