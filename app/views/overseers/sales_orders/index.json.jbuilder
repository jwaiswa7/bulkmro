  ...
  json.array! [
                  [
                      ...
                      if policy(sales_order).can_request_po?
                        row_action_button(new_overseers_po_request_path(:sales_order_id => sales_order.to_param), 'external-link', 'Request PO', 'success', :_blank)
                      end,
                      if policy(sales_order).can_request_invoice?
                        row_action_button(new_overseers_invoice_request_path(:sales_order_id => sales_order.to_param), 'file-invoice', 'Invoice Request', 'success', :_blank)
                      end
                  ].join(' '),
