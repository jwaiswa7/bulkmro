json.data (@customer_order_status_records) do |sales_order|
  ...
  json.merge! columns.merge("DT_RowClass": sales_order[:customer_delivery_date].present? && sales_order[:customer_delivery_date].to_date > Date.today ? 'bg-highlight-danger' : '')
end
