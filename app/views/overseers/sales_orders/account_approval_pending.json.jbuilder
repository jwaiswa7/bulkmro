json.data (@sales_orders) do |sales_order|
  json.array! [
                 [
                     row_action_button_without_fa(new_accounts_confirmation_overseers_inquiry_sales_order_path(sales_order.inquiry.to_param, sales_order.to_param), 'comment-alt-check', 'Account Approval', 'warning'),
                     row_action_button_without_fa(overseers_inquiry_sales_order_path(sales_order.inquiry, sales_order, format: :pdf), 'file-pdf', 'Download', 'dark', :_blank)
                 ],
