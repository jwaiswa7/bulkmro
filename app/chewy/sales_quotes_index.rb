...
class SalesQuotesIndex < BaseIndex
  ...
  define_type SalesQuote.limit(1).with_includes do
    ...
    field :sales_quote_rows do
      field :sku, value: ->(_record, sales_quote_row) { sales_quote_row.product.sku if sales_quote_row.product.sku.present? }, analyzer: 'substring'
      field :name, value: ->(_record, sales_quote_row) { sales_quote_row.product.name if sales_quote_row.product.name.present? }, analyzer: 'substring'
      field :brand, value: ->(_record, sales_quote_row) { sales_quote_row.product.brand.name if sales_quote_row.product.brand.present? }, analyzer: 'substring'
      field :mpn, value: ->(_record, sales_quote_row) { sales_quote_row.product.mpn if sales_quote_row.product.mpn.present? }, analyzer: 'substring'
    end
  end
end
