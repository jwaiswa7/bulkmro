class Overseers::PoRequestsController < Overseers::BaseController
  before_action :set_po_request, only: [:show, :edit, :update, :cancel_porequest, :render_cancellation_form, :render_comment_form, :render_modal_form, :add_comment, :new_purchase_order, :create_purchase_order, :manager_amended, :rejected_purchase_order, :reject_purchase_order_modal]
  ...
  def cancel_porequest
    ...
    elsif @po_request.status.blank?
      if @po_request.inquiry.inside_sales_owner.parent.present?
        @email_message = @po_request.purchase_order.email_messages.build(overseer: current_overseer , purchase_order: @po_request.purchase_order)
        @email_message.assign_attributes(
          to: @po_request.inquiry.inside_sales_owner.parent.email,
          subject: "Request for PO Cancellation has been generated by #{@po_request.inquiry.inside_sales_owner.full_name} for Inquiry##{@po_request.inquiry.inquiry_number} Purchase Order##{@po_request.purchase_order.metadata['PoNum']}",
          body: PoRequestMailer.cancel_po_request(@email_message).body.raw_source,
          auto_attach: false
          )
        if po_request_params[:attachments].present?
          @email_message.assign_attributes( files: po_request_params[:attachments])
        end
        ...
        render json: {error: {base: 'Procurement Specialist’s Parent not Exists '}}, status: 500
      end
    ...
    def po_request_params
      params.require(:po_request).permit(
        :id,
          ...
          rows_attributes: [:id, :sales_order_row_id, :product_id, :_destroy, :status, :quantity, :tax_code_id, :tax_rate_id, :measurement_unit_id,:brand_id, :discount_percentage, :unit_price, :lead_time, :converted_unit_selling_price, :product_unit_selling_price, :conversion, :bp_catalog_name, :bp_catalog_sku, :default_currency, :selected_currency_up],
          attachments: []
      )
    end
