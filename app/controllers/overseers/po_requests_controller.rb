class Overseers::PoRequestsController < Overseers::BaseController
  before_action :set_po_request, only: [:show, :edit, :update, :cancel_porequest, :render_cancellation_form, :render_comment_form, :render_modal_form, :add_comment, :new_purchase_order, :create_purchase_order, :manager_amended, :reject_purchase_order_modal, :render_modal_form_by_isp, :cancel_porequest_by_isp]
  before_action :set_notification, only: [:update, :cancel_porequest, :create_purchase_order, :manager_amended, :cancel_porequest_by_isp]
  ...
  def cancel_porequest_by_isp
    ...
    if @po_request.cancellation_reason.present?
      ...
      @email_message.assign_attributes(
        to: @po_request.inquiry.procurement_operations.parent.email,
        subject: "Request for PO Cancellation has been generated by #{@po_request.inquiry.procurement_operations.full_name} for Inquiry##{@po_request.inquiry.inquiry_number} Purchase Order##{@po_request.purchase_order.metadata['PoNum']}",
        body: PoRequestMailer.cancel_po_request(@email_message).body.raw_source,
        files: po_request_params[:attachments],
        auto_attach: false
      )
