class Overseers::Inquiries::SalesQuotesController < Overseers::Inquiries::BaseController
  ...
  def save
    ...
    service.call
    SalesQuoteMailer.with(overseer: current_overseer.id, inquiry: @inquiry, quote: @sales_quote).sales_quote_ack.deliver_now
  end
  ...
  def save_and_send
    ...
    service = Services::Overseers::SalesQuotes::ProcessAndSave.new(@sales_quote)
    service.call
    SalesQuoteMailer.with(overseer: current_overseer.id, inquiry: @inquiry, quote: @sales_quote).sales_quote_sent.deliver_now
  end
