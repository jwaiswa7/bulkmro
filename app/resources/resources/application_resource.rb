...
-----END RSA PRIVATE KEY-----'
  ...
  def self.find(id, quotes: false)
    ...
    OpenStruct.new(response.parsed_response) if response.class.name == "HTTParty::Response" && response.present? && response.parsed_response.present? && (response.parsed_response.is_a? Hash)
  end
