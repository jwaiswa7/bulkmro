...
-----END RSA PRIVATE KEY-----'
  ...
  def self.custom_find(id, by = nil)
    response = get("/#{collection_name}?$filter=#{by || identifier} eq '#{id}'&$top=1")
  ...
  def self.get_validated_response(raw_response)
    if raw_response['odata.metadata'] || (200...300).cover?(raw_response.code)
  ...
  def self.log_response(response)
    status = :success
    status = :failed if response[:error_message].present?
