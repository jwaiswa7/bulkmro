class Resources::Draft < Resources::ApplicationResource
  ...
  def self.to_remote(record)
    ...
    record.rows.each_with_index do |row, index|
      ...
      item.BaseLine = index #row.try(:sr_no) #row.sales_quote_row.sr_no #sale_order_row.sales_quote.line_num -----
...

    {
        ...
        BPChannelCode: record.inquiry.company.remote_uid,
        U_ConsigneeAddr: record.inquiry.shipping_address.to_s,
        U_Freight_Cost: record.inquiry.freight_cost,
        DocCurrency: record.inquiry.inquiry_currency,
        GroupNum: nil,
