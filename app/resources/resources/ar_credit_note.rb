class Resources::ArCreditNote < Resources::ApplicationResource

  SAP = OpenStruct.new(
    attachment_directory: Settings.sap.ATTACHMENT_DIRECTORY,
    attachment_api: Settings.credit_note.ATTACHMENT_API,
    server: {host: ATTACHMENT_ENDPOINT.host, port: ATTACHMENT_ENDPOINT.port},
    login: {user: Settings.credit_note.ATTACHMENT_USERNAME, password: Settings.credit_note.ATTACHMENT_PASSWORD},
    draft_doc_object_code: 17,
    draft_base_type: 23,
    attachment_username: Settings.credit_note.ATTACHMENT_USERNAME,
    ssh_key: ATTACHMENT_SSH
  )

  def self.ar_credit_memo
    set_headers
    response = HTTParty.get(
       Settings.credit_note.ATTACHMENT_ENDPOINT,
         body: {CompanyDB: Settings.credit_note.DATABASE, UserName: Settings.credit_note.USERNAME, Password: Settings.credit_note.PASSWORD}.to_json,
         verify: false,
         debug_output: $stdout,
         timeout: 30,
     )
  ...
  ENDPOINT = URI.parse(Settings.credit_note.ATTACHMENT_ENDPOINT)
