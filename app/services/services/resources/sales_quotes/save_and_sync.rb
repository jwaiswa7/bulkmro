class Services::Resources::SalesQuotes::SaveAndSync < Services::Shared::BaseService
  ...
  def call_later
    if sales_quote.remote_uid.blank? && sales_quote.inquiry.quotation_uid.present? && sales_quote.parent.present? && sales_quote.parent.sales_orders.where.not(order_number: nil).blank?
      sales_quote.update_attributes(remote_uid: sales_quote.inquiry.quotation_uid)
    end
