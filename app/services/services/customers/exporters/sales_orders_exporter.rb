class Services::Customers::Exporters::SalesOrdersExporter < Services::Customers::Exporters::BaseExporter
  def initialize(headers, company, filters = nil)
    ...
    if company.id == 1847
      ...
      @start_at = filters['date_range'].present? ? filters['date_range'].split('~').first.to_date.strftime('%Y-%m-%d') : start_at
      @end_at = filters['date_range'].present? ? filters['date_range'].split('~').last.to_date.strftime('%Y-%m-%d') : end_at
