class Services::Customers::Exporters::SalesOrdersExporter < Services::Customers::Exporters::BaseExporter
  ...
  def build_csv
    Enumerator.new do |yielder|
      ...
      if company.id == 1847
        flex_offline_orders = SalesOrder.remote_approved.joins(:inquiry, :company).where(companies: {id: company.id}).where(inquiries: {customer_order_date: @start_at..@end_at}).order(name: :asc)
        ...
        flex_online_orders = CustomerOrder.not_rejected.joins(:company).where(companies: {id: company.id}).where(created_at: @start_at..@end_at).order(name: :asc)
        ...
        online_orders.each do |customer_order|
          customer_order.rows.each do |record|
            ...
            rows.push(
              ...
              part_number: record.product.sku,
              account: inquiry.present? ? inquiry.company.account.name : Company.find(customer_order.company_id).account.name,
              line_item_quantity: record.quantity,
              line_item_net_total: record.customer_product.customer_price.to_f * record.quantity,
              order_status: '',
              user_email: inquiry.present? ? inquiry.contact.email.to_s : Contact.find(customer_order.contact_id).email.to_s,
