class Services::Customers::Charts::MixedChart < Services::Shared::Charts::ChartConfig
  ...
  def customers_quarterly_purchase_data(company)
    ...
    build_chart do
      ...
      so = SalesOrder.includes(:rows).remote_approved.where(:created_at => start_at..end_at).joins(:company).where(companies: {id: company.id})
