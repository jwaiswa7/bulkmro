class Services::Customers::Finders::SalesQuotes < Services::Customers::Finders::BaseFinder
  ...
  def perform_query(query_string)
    ...
    indexed_records = index_klass.query({multi_match: {query: query_string, operator: 'and', fields: %w[inquiry_number_string^3 sales_person  line_items valid_upto_s status_string quote_total ]}}).order(sort_definition)
