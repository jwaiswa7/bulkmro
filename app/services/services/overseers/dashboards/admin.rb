class Services::Overseers::Dashboards::Admin < Services::Shared::BaseService
  ...
  def call
    ...
    sales_invoices = SalesInvoice.not_cancelled.includes(:rows).where(:created_at => start_at.beginning_of_month..end_at.end_of_month).where.not(sales_order_id: nil).where.not(metadata: nil)
