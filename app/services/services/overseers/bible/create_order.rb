class Services::Overseers::Bible::CreateOrder < Services::Shared::BaseService
  ...
  def call
    @bible_upload_queue = BibleFileUpload.where(status: 'Pending')
    @bible_upload_queue.each do |upload_sheet|
      ...
      service.loop(nil) do |x|
        begin
          ...
          upload_sheet.bible_upload_logs.create(sr_no: i, bible_row_data: x.get_row.to_json, status: false, error: error.message)
        end
      end
      calculate_totals
