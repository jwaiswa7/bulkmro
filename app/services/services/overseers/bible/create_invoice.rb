class Services::Overseers::Bible::CreateInvoice < Services::Shared::BaseService
  def initialize
  end

  def test
    service = Services::Shared::Spreadsheets::CsvImporter.new('bible_invoice_till_june.csv', 'seed_files_3')
    service.loop(nil) do |x|
      bible_total = x.get_column('Total Selling Price').to_f
      bible_total_with_tax = x.get_column('Total Selling Price').to_f + x.get_column('Tax Amount').to_f

      bible_invoice = BibleInvoice.where(inquiry_number: x.get_column('Inquiry Number').to_i,
                                          order_number: x.get_column('So #'),
                                          mis_date: Date.parse(x.get_column('Order Date')).strftime('%Y-%m-%d')).first_or_create! do |bible_invoice|
        bible_invoice.inside_sales_owner = x.get_column('Inside Sales Name')
        bible_invoice.company_name = x.get_column('Magento Company Name')
        bible_invoice.account_name = x.get_column('Company Alias')
        bible_invoice.client_order_date = Date.parse(x.get_column('Client Order Date')).strftime('%Y-%m-%d') if x.get_column('Inquiry Number') != '31647'
        bible_invoice.currency = x.get_column('Price Currency')
        bible_invoice.document_rate = x.get_column('Document Rate')
        bible_invoice.metadata = []
      end

      if bible_invoice.present?
        skus_in_order = bible_order.metadata.map {|h| h['sku']}
        puts 'SKU STATUS', skus_in_order.include?(x.get_column('Bm #'))

        order_metadata = bible_invoice.metadata
        sku_data = {
            'sku': x.get_column('Bm #'),
            'description': x.get_column('Description'),
            'quantity': x.get_column('Order Qty'),
            'freight': x.get_column('Freight'),
            'total_landed_cost': x.get_column('Total Landed Cost(Total buying price)').to_f,
            'unit_cost_price': x.get_column('unit cost price').to_f,
            'unit_selling_price': x.get_column('Unit selling Price').to_f,
            'total_selling_price': x.get_column('Total Selling Price').to_f,
            'tax_type': x.get_column('Tax Type'),
            'tax_rate': x.get_column('Tax Rate'),
            'tax_amount': x.get_column('Tax Amount').to_f,
            'total_selling_price_with_tax': x.get_column('Total Selling Price').to_f + x.get_column('Tax Amount').to_f,
            'margin_percentage': x.get_column('Margin (In %)'),
            'margin_amount': x.get_column('Margin').to_f,
            'order_date': Date.parse(x.get_column('Order Date')).strftime('%Y-%m-%d')
        }
        order_metadata.push(sku_data)
        bible_invoice.assign_attributes(metadata: order_metadata)
        bible_invoice.save
      end
    end

    calculate_totals
    puts 'BibleSO', BibleSalesOrder.count
  end

  def calculate_totals
    BibleInvoice.all.each do |bible_invoice|
      @bible_order_total = 0
      @bible_order_tax = 0
      @bible_order_total_with_tax = 0
      @order_margin = 0

      bible_invoice.metadata.each do |line_item|
        @bible_order_total = @bible_order_total + line_item['total_selling_price']
        @bible_order_tax = @bible_order_tax + line_item['tax_amount']
        @bible_order_total_with_tax = @bible_order_total_with_tax + line_item['total_selling_price_with_tax']
        @order_margin = @order_margin + line_item['margin_amount']
      end
      bible_invoice.update_attributes(order_total: @bible_order_total, order_tax: @bible_order_tax, order_total_with_tax: @bible_order_total_with_tax, total_margin: @order_margin)
    end
  end
end