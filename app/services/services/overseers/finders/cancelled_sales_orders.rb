class Services::Overseers::Finders::CancelledSalesOrders < Services::Overseers::Finders::BaseFinder
  def call
    call_base
  end
  ...
  def perform_query(query_string)
    ...
    if current_overseer.present? && !current_overseer.allow_inquiries?
      indexed_records = indexed_records.filter(filter_by_owner(current_overseer.self_and_descendant_ids).merge(filter_by_value(:status, SalesOrder.statuses['Cancelled'])))
    else
      indexed_records = indexed_records.filter(filter_by_value(:status, SalesOrder.statuses['Cancelled']))
    end
