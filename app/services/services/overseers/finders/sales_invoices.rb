class Services::Overseers::Finders::SalesInvoices < Services::Overseers::Finders::BaseFinder
  ...
  def perform_query(query_string)
    indexed_records = index_klass.query(multi_match: { query: query_string, operator: 'and', fields: %w[invoice_number_string^3 cp_po_number_s  sales_order_number_string account_string company_string^4 status_string inquiry_number_string inside_sales_owner outside_sales_owner] }).order(sort_definition).order(sort_definition)
