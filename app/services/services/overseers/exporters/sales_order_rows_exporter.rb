  ...
  def call
    model.joins(:sales_order).where('sales_orders.status = ?', SalesOrder.statuses['Approved']).where(:created_at => start_at..end_at).each do |row|
      ...
      rows.push({
                    ...
                    :Supplier_Domestic_Imports => (if (inquiry.company.default_shipping_address.try(:country_code) == "IN" || sales_quote_row.supplier.addresses.first.try(:country_code) == "IN") then "Domestic" else "Imports" end),
                    :Oem_Non_Oem => "",
                    :Revenue_Stream => "",
                    :Business_Vertical => "",
                    :HighValue_LowValue => "",
