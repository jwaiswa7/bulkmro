class Services::Overseers::Exporters::CustomerSalesOrdersExporter < Services::Overseers::Exporters::BaseExporter

  def initialize
    super
    @export_name = 'customer_sales_orders'
    @company = Thread.current[:current_company]
  ...
  def call
    perform_export_later('CustomerSalesOrdersExporter')
  end

  def build_csv
    SalesOrder.remote_approved.where.not(sales_quote_id: nil).joins(:inquiry).where('inquiries.company_id = ?', company.id).where(mis_date: start_at..end_at).order(mis_date: :desc).each do |sales_order|
