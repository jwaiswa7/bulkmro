  ...
  def call
    model.status_Approved.where(:created_at => start_at..end_at).each do |sales_order|
      ...
      rows.push({
                    ...
                    :company_alias => if inquiry.try(:account) then inquiry.account.try(:name) else "" end,
                    :company_name => if inquiry.try(:company) then inquiry.company.try(:name).gsub(/;/, ' ') else "" end,
