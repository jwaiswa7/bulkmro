  ...
  def build_csv
    ...
    records.each do |inquiry|
      rows.push(
        ...
        sales_quotes: number_with_delimiter(inquiry['sales_quotes']['value']),
        total_quote_value: format_currency(inquiry['total_sales_value']['value']),
        expected_orders: number_with_delimiter(inquiry['expected_orders']['value']),
        total_expected_orders_value: format_currency(inquiry['total_order_value']['value']),
        sales_orders: number_with_delimiter(inquiry['sales_orders']['value']),
        total_sales_orders: format_currency(inquiry['total_sales_value']['value']),
        sku: number_with_delimiter(inquiry['sku']['value']),
        inquiries_won: inquiry['orders_won']['value'] > 0 ? percentage(inquiry['orders_won']['value'] * 100.0 / inquiry['doc_count']) : '-',
        sales_invoices: number_with_delimiter(inquiry['sales_invoices']['value'], delimiter: ','),
        revenue: format_currency(inquiry['revenue']['value']),
        clients: number_with_delimiter(inquiry['clients']['value'])
          ) if inquiry.present?
    end
