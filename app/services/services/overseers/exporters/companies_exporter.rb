  ...
  def build_csv
    model.includes({ addresses: :state }, :company_contacts, :inside_sales_owner, :outside_sales_owner, :industry, :account).all.order(created_at: :desc).each do |record|
      rows.push(
        ...
        inside_sales_owner: (record.inside_sales_owner.full_name if record.inside_sales_owner.present?),
        outside_sales_owner: (record.outside_sales_owner.full_name if record.outside_sales_owner.present?),
