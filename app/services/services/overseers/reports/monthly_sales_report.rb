class Services::Overseers::Reports::MonthlySalesReport < Services::Overseers::Reports::BaseReport
  def call
    ...
    sales_orders = SalesOrder.includes([rows: :sales_quote_row]).where(:mis_date => start_at.beginning_of_month..end_at.end_of_month).where('sales_orders.status = ? OR sales_orders.legacy_request_status = ?', SalesOrder.statuses[:'Approved'], SalesOrder.statuses[:'Approved'])
    ...
    months.each do |month|
      ...
      orders = sales_orders.where(:mis_date => month.to_date.beginning_of_month..month.to_date.end_of_month)
      ...
      @data.entries[month.to_s][:total] = orders.map{|s| s.report_total}.inject(0){|sum,x| sum + x }.to_f.round(2)
