class Services::Overseers::ArInvoiceRequests::New < Services::Shared::BaseService
  ...
  def call
    ...
    @sales_order_rows.each do |sales_order_row|
      ...
      ar_invoice_request_rows = ArInvoiceRequestRow.where(sales_order_id: sales_order_id, product_id: product_id).joins(:ar_invoice_request).where.not(ar_invoice_requests: {status: "Cancelled AR Invoice"}).where(ar_invoice_requests: {inward_dispatch_ids: inward_dispatches.pluck(:id)})
