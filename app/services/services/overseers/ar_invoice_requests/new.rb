  ...
  def call
    ...
    inward_dispatches.each do |inward_dispatch|
      inward_dispatch.rows.each do |row|
        @ar_invoice_request.rows.build( delivered_quantity: row.delivered_quantity, quantity: row.delivered_quantity, inward_dispatch_row_id: row.id, sales_order_id: @sales_order.id )
      end
    end
    @ar_invoice_request
  end
