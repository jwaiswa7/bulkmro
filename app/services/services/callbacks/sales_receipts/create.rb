class Services::Callbacks::SalesReceipts::Create < Services::Callbacks::Shared::BaseCallback
  def call
    ...
    sales_receipt = SalesReceipt.where(remote_reference: params['p_sap_reference_number']).first_or_create! do |sales_receipt_objrct|
      sales_receipt_objrct.assign_attributes(
        company: company,
        account: account,
        metadata: params,
        currency: currency,
