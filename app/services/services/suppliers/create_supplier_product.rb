class Services::Suppliers::CreateSupplierProduct < Services::Shared::BaseService
  ...
  def call
    ...
    unless supplier_products.present?
      supplier_product_params = { supplier_id: supplier_id, product_id: product.id, brand_id: product.brand_id, category_id: product.category_id, name: product.name, sku: product.sku,
                                  supplier_price: unit_price, unit_selling_price: unit_price, tax_rate_id: product.tax_rate_id, tax_code_id: product.tax_code_id,
                                  measurement_unit_id: product.measurement_unit_id, inquiry_ids: [inquiry_product_supplier.inquiry_id] }
      @supplier_product = SupplierProduct.new(supplier_product_params)
      @supplier_product.save
    else
      supplier_products.each do |supplier_product|
        supplier_product.inquiry_ids.push(inquiry_product_supplier.inquiry_id) unless supplier_product.inquiry_ids.include?(inquiry_product_supplier.inquiry_id)
        supplier_product.save
      end
    end
  end
