...
#     105
  ...
  def set_roles
    [
        ...
        ['admin', 'bhargav.trivedi@bulkmro.com'],
  ...
  def check_and_fix_sales_quote_row_margins
    ...
    SalesOrderRow.where(sales_order: SalesOrder.remote_approved).where.not(sales_quote_row: nil).includes(:sales_order, :product, :sales_quote_row, :inquiry_product_supplier).each do |sales_order_row|
      ...
      if is_not_valid
        ...
        old_margin_percentage = sales_quote_row.margin_percentage
        ...
        if old_margin_percentage.round(2) != margin_percentage.round(2)
          # sales_quote_row.update_attribute(:margin_percentage, margin_percentage)
          sku = sales_quote_row.product.sku
          ...
          values << [order_number, sales_order_row.id, sku, old_margin_percentage, margin_percentage, unit_cost_price_with_unit_freight_cost.round(4), unit_selling_price].map(&:to_s)
        end
    ...
    [values, invalid_sales_orders.count, invalid_sales_orders_rows.count, fixed_sales_orders_rows.count]
  end
