class Services::Shared::Migrations::MigrationsV2 < Services::Shared::Migrations::Migrations
  ...
  def sales_quote_blank
    ...
    message = "The Quote date was updated from Backend on per request from Nillesh Desai and Nutan Bala. Note - there is no actual quote in the system as these were sent from email and out of system."
    ...
    csv_data = CSV.generate(write_headers: true, headers: column_headers) do |writer|
      service.loop(nil) do |x|
        Inquiry.where(inquiry_number: x.get_column('inquiry_number'), quotation_date: nil).each do |inquiry|
          inquiry.update_attributes(quotation_date:  x.get_column('Quote_date'))
