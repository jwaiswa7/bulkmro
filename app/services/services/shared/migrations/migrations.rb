  ...
  def create_missing_orders
    ...
    service.loop(nil) do |x|
      ...
      if inquiry.present?
        ...
        row = nil
        if inquiry.sales_orders.where(order_number: x.get_column('order number').to_i).present?
        ...
        tax_rate = TaxRate.where(tax_percentage: x.get_column('tax rate').to_f).first_or_create! row.unit_selling_price = x.get_column('unit selling price (INR)').to_f
