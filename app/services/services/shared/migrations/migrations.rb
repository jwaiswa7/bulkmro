  ...
  def create_missing_invoices
    ...
    service.loop(nil) do |x|
      ...
      if sales_order.present?
        ...
        # rows
        row = nil
        if invoice.rows.map {|si_row| si_row.metadata['sku']}.include?(x.get_column('BM #'))
          row = invoice.rows.where("metadata->>'sku' = ?", x.get_column('BM #')).first
        end
        ...
        missing_bible_orders.push(x.get_column('SO #'))
