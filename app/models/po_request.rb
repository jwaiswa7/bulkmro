...
class PoRequest < ApplicationRecord
  ...
  pg_search_scope :locate, against: [:id], associated_against: { sales_order: %i[id order_number], inquiry: [:inquiry_number] }, using: { tsearch: { prefix: true } }

  belongs_to :sales_order
  belongs_to :inquiry
  belongs_to :logistics_owner, ->(_record) { where(role: 'logistics') }, class_name: 'Overseer', foreign_key: 'logistics_owner_id'
  ...
  def purchase_order_created?
    if status == 'PO Created' && purchase_order.blank?
