module Mixins::HasConvertedCalculations
  ...
  included do
    ...
    def calculated_total_with_tax_for_tcs_applicable
      rows.map { |row| row.product.tcs_applicable ? row.total_selling_price_with_tax : 0 }.compact.sum.round(2)
    end
    ...
    def tcs_amount
      (calculated_total_for_tcs_without_service.to_f * ((Settings.tcs.tcs_rate).to_i / 100))
    end
