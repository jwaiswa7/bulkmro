...
class PurchaseOrder < ApplicationRecord
  ...
  pg_search_scope :locate, against: %i[id po_number], using: { tsearch: { prefix: true } }
  ...
  def get_supplier(product_id)
    if metadata['PoSupNum'].present?
      product_supplier = (Company.find_by_legacy_id(metadata['PoSupNum']) || Company.find_by_remote_uid(metadata['PoSupNum']))
      return product_supplier if inquiry.suppliers.include?(product_supplier) || is_legacy?
    end

    if inquiry.final_sales_quote.present?
      product_supplier = inquiry.final_sales_quote.rows.select { |supplier_row|  supplier_row.product.id == product_id || supplier_row.product.legacy_id == product_id }.first
      return product_supplier.supplier if product_supplier.present?
    end
  end

  def metadata_status
    PurchaseOrder.statuses.key(metadata['PoStatus'].to_i).to_s if metadata.present?
  end

  def to_s
    supplier_name = get_supplier(rows.first.metadata['PopProductId'].to_i) if rows.present?
    ['#' + po_number.to_s, supplier_name].join(' ') if po_number.present?
  end

  def valid_po_date?
    metadata['PoDate'].to_date
    true
