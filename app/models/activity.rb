...
class Activity < ApplicationRecord
  ...
  pg_search_scope :locate, against: %i[purpose company_type activity_type], associated_against: { created_by: %i[first_name last_name], account: [:name], company: [:name], contact: %i[first_name last_name], inquiry: [:inquiry_number] }, using: { tsearch: { prefix: true } }

  has_many :activity_overseers
  has_many :overseers, through: :activity_overseers
  accepts_nested_attributes_for :activity_overseers, reject_if: ->(attributes) { attributes['overseer_id'].blank? && attributes['id'].blank? }, allow_destroy: true
